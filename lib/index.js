...
function compiler(telegrafBotByUser, conf = {}) {
    ...
    let stage = new telegraf_1.Scenes.Stage([cScene, pyScene, jsScene, cppScene, jvScene, goScene, tsScene, shScene, psScene], { ttl: config_1.default.ttl });
    ...
    bot.hears(new RegExp("^\\" + config_1.default.startSymbol + "(code|start|py|python|ts|type|js|node|cc|cpp|cplus|sql|go|jv|java|c\\+\\+|sh|ps)|\\/start", "i"), async (ctx, next) => {
        try {
            if (conf.allowed) {
